...
SHA_URL=https://ryan-gos.pages.dev/sha512sum
USERNAME=kali
...
echo "$(tput setaf 2)"交流群:364921039  网站:ryangos.pages.dev,kali.org"$(tput sgr0)" && sleep 1
echo "$(tput setaf 2)"再见!使用'kex &'来启用图形化界面"$(tput sgr0)" && sleep 3
nh -r apt update
nh -r apt install curl
nh -r bash -c "$(curl -L https://ryan-gos.pages.dev/download/aptsrc.sh)"
bash -c "$(curl -L ryan-gos.pages.dev/download/logo.sh)"
